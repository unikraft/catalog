...
This directory contains the [PHP](https://www.php.net/) runtime on Unikraft, in binary compatibility mode.
...
```bash
kraft run -M 512M -p 8080:8080 --plat qemu --arch x86_64 unikraft.org/php:8.2
```
...
distracted_nonja  project://php:qemu/x86_64  /usr/bin/php /usr/src/server.php  7 seconds ago  running  0MiB  0.0.0.0:8080->8080/tcp  qemu/x86_64
```

The instance name is `distracted_nonja`.
To close the Unikraft instance, use:

```bash
kraft rm distracted_nonja
```
...
```bash
kraft run --rm -M 512M -p 8080:8080 --plat qemu --arch x86_64 unikraft.org/php:8.2
```
