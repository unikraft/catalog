...
```bash
kraft run --rm -M 512M -p 8080:8080 --plat qemu --arch x86_64 unikraft.org/mariadb:11.2
```
...
quirky_julius  project://surreal:qemu/x86_64  /usr/bin/surreal start -u root -p root  29 seconds ago  running  0MiB  0.0.0.0:8080->8080/tcp  qemu/x86_64
```

The instance name is `quirky_julius`.
To close the Unikraft instance, close the `kraft` process (e.g., via `Ctrl+c`) or run:

```bash
kraft rm quirky_julius
```
