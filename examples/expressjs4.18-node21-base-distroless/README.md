...
clever_louis  oci://unikraft.org/base:latest  /usr/bin/node /usr/src/server.js  3 seconds ago  running  976M  0.0.0.0:3000->3000/tcp  qemu/x86_64
```

The instance name is `clever_louis`.
To close the Unikraft instance, close the `kraft` process (e.g., via `Ctrl+c`) or run:

```bash
kraft rm clever_louis
```
