#!/usr/bin/env bash
# vegeta load test for the Go HTTP server.
# Server must already be running (just run-qemu or just run-fc).
# QEMU and Firecracker both expose the server at the same guest IP.

set -euo pipefail

BASE_URL="${BASE_URL:-http://172.44.0.2:8080}"
