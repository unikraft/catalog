...
reverent_humphrey  oci://unikraft.org/base:latest  /usr/local/bin/python3 /app/serve...  3 minutes ago  running  488M  0.0.0.0:8080->8080/tcp  qemu/x86_64
```

The instance name is `reverent_humphrey`.
To close the Unikraft instance, close the `kraft` process (e.g., via `Ctrl+c`) or run:

```bash
kraft rm reverent_humphrey
```
