...
This directory contains a C-based HTTP server example compiled with GCC 15.2 running on Unikraft using the gcr.io/distroless/cc-debian12 base image. This minimal environment lacks a shell and unnecessary system utilities, significantly reducing the attack surface.
...
blissful_pocketswarhol  oci://unikraft.org/base:latest  /http_server  3 seconds ago  running  122M  0.0.0.0:8080->8080/tcp  qemu/x86_64
```

The instance name is `blissful_pocketswarhol`.
To close the Unikraft instance, close the `kraft` process (e.g., via `Ctrl+c`) or run:

```bash
kraft rm blissful_pocketswarhol
```
