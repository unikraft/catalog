# Rust Actix Web Server - Distroless
...
nice_jackie  oci://unikraft.org/base:latest  /server  14 seconds ago  running  128M  0.0.0.0:8080->8080/tcp  qemu/x86_64
```

The instance name is `nice_jackie`.
To close the Unikraft instance, close the `kraft` process (e.g., via `Ctrl+c`) or run:

```bash
kraft rm nice_jackie
```
